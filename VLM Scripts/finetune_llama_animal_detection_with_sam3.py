...
def build_user_prompt_with_sam3(
    use_night_time: bool, night_time: str, use_sam3_assist: bool
) -> str:
    base = build_user_prompt(use_night_time=use_night_time, night_time=night_time)
    if not use_sam3_assist:
        return base
    return (
        f"{base} Two related images are provided: first is original (primary), "
        "second is SAM3-assisted reference. Prioritize the original image."
    )


def build_user_content(user_prompt: str, use_sam3_assist: bool) -> list[dict[str, str]]:
    if use_sam3_assist:
        return [
            {"type": "image"},
            {"type": "image"},
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="End-to-end fine-tuning + evaluation for Llama-3.2-11B-Vision-Instruct on camera-trap labels (Qwen SAM3 setup compatibility)."
    )
    parser.add_argument(
        "--model-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="/storage/ice-shared/cs8903onl/Mcquire-animal-detection/VLM/llama_finetune_output_sam3_new_version",
    )
    parser.add_argument(
        "--eval-ratio",
        type=float,
    ...
    parser.add_argument(
        "--disable-sam3-assist",
        action="store_true",
        help="Use only original image as model input (ignore SAM3 assistant image).",
    )
    parser.add_argument(
        "--time-windows",
        default="30,90",
...
def load_examples_from_split_json(path: str, shared_root: str) -> list[dict[str, Any]]:
    ...
    # SAM3 split JSON can include file_path_original/file_path_sam3 or separate
    # original/sam3 variant rows. Merge to one sample with original + optional SAM3.
    has_variant_rows = any(
        ("image_variant" in row) or ("file_path_original" in row) or ("file_path_sam3" in row)
        for row in rows
    )
    if not has_variant_rows:
        out = []
        for row in rows:
    ...
    for row in rows:
        raw_original = row.get("file_path_original") or row.get("file_path", "")
        original_fp = remap_image_path(raw_original, shared_root)
        if not original_fp:
            continue

        raw_sam3 = row.get("file_path_sam3", "")
        sam3_fp = remap_image_path(raw_sam3, shared_root) if raw_sam3 else ""
        variant = str(row.get("image_variant", "")).strip().lower()
        if variant == "sam3":
            variant_fp = remap_image_path(row.get("file_path", ""), shared_root)
            if variant_fp and variant_fp != original_fp:
                sam3_fp = sam3_fp or variant_fp

        night_time = "Y" if str(row.get("night_time", "N")).strip().upper() == "Y" else "N"
        split_group_id = row.get("split_group_id", original_fp)
        burst_window_used = int(row.get("burst_window_used", 0) or 0)
        target = row.get("correct_label", "unknown")
        key = (original_fp, target, night_time, burst_window_used, split_group_id)
        ...
        if key not in grouped:
            grouped[key] = {
                ...
                "burst_window_used": burst_window_used,
                "split_group_id": split_group_id,
            }
        if sam3_fp and sam3_fp != original_fp:
            grouped[key]["sam3_file_path"] = sam3_fp
...
class MultiModalCollator:
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        for feat in features:
            orig_image = Image.open(feat["file_path"]).convert("RGB")
            if self.use_sam3_assist:
                sam3_path = feat.get("sam3_file_path", "")
                if sam3_path and os.path.exists(sam3_path):
                    sam3_image = Image.open(sam3_path).convert("RGB")
                else:
                    sam3_image = orig_image.copy()
                images.append([orig_image, sam3_image])
            else:
                images.append(orig_image)
            user_prompt = build_user_prompt_with_sam3(
                use_night_time=self.use_night_time,
                night_time=feat.get("night_time", "N"),
                use_sam3_assist=self.use_sam3_assist,
            )
            ...
            user_messages = [
                {
                    ...
                    "content": build_user_content(
                        user_prompt=user_prompt, use_sam3_assist=self.use_sam3_assist
                    ),
                }
            ]
            full_messages = [
...
def load_model_and_processor(args: argparse.Namespace) -> tuple[Any, Any]:
    ...
    if args.use_4bit:
        ...
        # In DDP mode each rank should pin the quantized model to its local GPU.
...
def evaluate_generation(
    ...
    for row in eval_rows:
        orig_image = Image.open(row["file_path"]).convert("RGB")
        if use_sam3_assist:
            sam3_path = row.get("sam3_file_path", "")
            if sam3_path and os.path.exists(sam3_path):
                sam3_image = Image.open(sam3_path).convert("RGB")
            else:
                sam3_image = orig_image.copy()
            sample_images: Any = [[orig_image, sam3_image]]
        else:
            sample_images = [orig_image]
        user_prompt = build_user_prompt_with_sam3(
            use_night_time=use_night_time,
            night_time=row.get("night_time", "N"),
            use_sam3_assist=use_sam3_assist,
        )
        user_messages = [
            {
                "role": "user",
                "content": build_user_content(
                    user_prompt=user_prompt, use_sam3_assist=use_sam3_assist
                ),
            }
        ]
        text = processor.apply_chat_template(user_messages, tokenize=False, add_generation_prompt=True)
        inputs = processor(text=[text], images=sample_images, return_tensors="pt")
        ...
        details.append(
            {
                ...
                "sam3_file_path": row.get("sam3_file_path", ""),
...
def main() -> None:
    ...
    use_presplit = bool(args.train_json and args.validation_json)
    if use_presplit:
        ...
        test_rows = (
            load_examples_from_split_json(args.test_json, args.shared_root) if args.test_json else []
        )
    ...
    data_collator = MultiModalCollator(
        ...
        use_sam3_assist=not args.disable_sam3_assist,
    )
        ...
        if resume_arg.lower() == "auto":
            ...
            for child in checkpoint_dir.glob("checkpoint-*"):
                ...
                if step > latest_step:
                    latest_step = step
                    latest_path = child
            resume_checkpoint = str(latest_path) if latest_path else None
    ...
    if world_size > 1 and not is_main_process():
        # Avoid duplicated generation/evaluation writes from non-zero ranks.
        return
