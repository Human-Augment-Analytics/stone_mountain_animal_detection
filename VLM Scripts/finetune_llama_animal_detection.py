...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="End-to-end fine-tuning + evaluation for Llama-3.2-11B-Vision-Instruct on camera-trap labels."
    )
    parser.add_argument(
        "--model-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="/storage/ice-shared/cs8903onl/Mcquire-animal-detection/VLM/llama_finetune_output",
    )
    parser.add_argument(
        "--eval-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Checkpoint path, 'auto' to pick latest checkpoint in output/checkpoints, or 'none'.",
    )
...
def load_model_and_processor(args: argparse.Namespace) -> tuple[Any, Any]:
    ...
    if args.gradient_checkpointing:
        gc_enable_sig = inspect.signature(model.gradient_checkpointing_enable)
        if "gradient_checkpointing_kwargs" in gc_enable_sig.parameters:
...
def main() -> None:
    ...
    if not args.skip_train:
        ...
        resume_arg = (args.resume_from_checkpoint or "auto").strip().lower()
        if resume_arg == "auto":
            resume_from_checkpoint = find_latest_checkpoint(checkpoint_root)
        ...
        if resume_from_checkpoint:
            ...
            print("Starting training from scratch (no checkpoint found/requested).")
            trainer.train()
